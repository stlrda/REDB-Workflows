CREATE TABLE IF NOT EXISTS "core"."parceltest" (
...
WITH ID_Table AS
	...
	FROM (
		SELECT "ParcelId", "OwnerName", "OwnerName2", "address_id", "OwnerAddr", "OwnerCity", "OwnerState", "OwnerCountry", "OwnerZIP" 
	...
	LEFT JOIN "core"."legal_entitytest"
        ON COALESCE("OwnerName", ' ') = COALESCE("legal_entity_name", ' ')
        AND COALESCE("OwnerName2", ' ') = COALESCE("legal_entity_secondary_name", ' ')
        AND ("legal_entitytest"."address_id" = "qry"."address_id")
	)
INSERT INTO "core"."parceltest" ("parcel_id",
								"county_id",
								"city_block_number",
								"parcel_number",
	...
	(SELECT CONCAT('10001.',"parcel_number",'.000.0000'),
			'10001',
			"CityBlock",
			"parcel_number",
            -- "parcel_taxing_status"
			...
			"Precinct10",
            "InspArea10",
            "neighborhoodtest"."neighborhood_id",
            "PoliceDist",
            "CensTract10",
            "AsrNbrhd",
            "SpecParcelType",
	...
	FROM "staging_1"."prcl_prcl"
		...
		JOIN "core"."neighborhoodtest"
		ON "prcl_prcl"."Nbrhd" = "neighborhoodtest"."neighborhood_name"
	);

----------------------------old version using sequence to create the IDs----------------------------------------------------
...
-- INSERT INTO "core"."parceltest" ("parcel_id",
	...
	(SELECT CONCAT('10001.',(nextval('core.parcelid')),'.000.0000'),
			'10001',
			"CityBlock",
			(currval('core.parcelid')),
            -- "parcel_taxing_status"
			ID_Table."legal_entity_id",
			CONCAT("LegalDesc1",' ',"LegalDesc2",' ',"LegalDesc3",' ',"LegalDesc4",' ',"LegalDesc5"),
            CAST("Frontage" AS FLOAT),
